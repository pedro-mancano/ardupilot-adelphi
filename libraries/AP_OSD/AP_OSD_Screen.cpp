...
void AP_OSD_Screen::draw_horizon(uint8_t x, uint8_t y)
{
    ...
    int horizon_length = 6; // Default: 4

    if (fabsf(ky) < fabsf(kx))
    {
        for (int dx = -horizon_length; dx <= horizon_length; dx++)
        {
            ...
            if (dy >= -horizon_length && dy <= horizon_length)
            {
